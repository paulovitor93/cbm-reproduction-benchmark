...
def main(epochs=100, batch_size=32, SEED=42, train_dataset=None, val_dataset=None, fold_id=None, dataset_name="cub"):
    """
    Train a Joint Concept Bottleneck Model.
    """
    # CONFIG
    LEARNING_RATE = 1e-4
    LAMBDA_C = 1.0
    ...
    print(f"{dataset_name.upper()} JOINT_CV TRAINING")
    ...
    history_path = os.path.join(RESULTS_DIR, f"history_joint_cv_fold{fold_id}_seed{SEED}.csv")
    history.to_csv(history_path, index=False)
    print(f"Training history saved to {history_path}")

    results_file = os.path.join(RESULTS_DIR, f"training_summary_joint_cv_fold{fold_id}_seed{SEED}.txt")
