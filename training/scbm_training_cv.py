...
def main(epochs=100, batch_size=32, SEED=42, train_dataset=None, val_dataset=None, fold_id=None, dataset_name="cub"):
    ...
    print(f"{dataset_name.upper()} SCBM_CV TRAINING")
    ...
    train_precision_losses = []
    val_precision_losses = []
    ...
    model = SCBM(backbone=backbone, feature_dim=512, num_concepts=train_dataset.num_concepts,
                 num_classes=train_dataset.num_classes, mc_samples=50,).to(DEVICE)
    # LOSSES
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in train_bar:
            ...
            concept_probs_samples = outputs["concept_probs_samples"]  # (B, C, M)
            ...
            # Predictions
            c_pred = (concept_probs > 0.5)
            y_pred = class_logits.argmax(dim=1)
            ...
            train_precision_loss += precision_loss.item()
        ...
        train_precision_loss_epoch = train_precision_loss / len(train_loader)
        ...
        train_precision_losses.append(train_precision_loss_epoch)
        ...
        with torch.no_grad():
            ...
            for batch in val_bar:
                ...
                val_precision_loss += precision_loss.item()
        ...
        val_precision_loss_epoch = val_precision_loss / len(val_loader)
        ...
        val_precision_losses.append(val_precision_loss_epoch)
    ...
    history = pd.DataFrame({
        ...
        "train_precision_loss": train_precision_losses,
        "val_precision_loss": val_precision_losses,
    ...
    history_path = os.path.join(RESULTS_DIR, f"history_scbm_cv_fold{fold_id}_seed{SEED}.csv")
    history.to_csv(history_path, index=False)
    print(f"Training history saved to {history_path}")

    results_file = os.path.join(RESULTS_DIR, f"training_summary_scbm_cv_fold{fold_id}_seed{SEED}.txt")
