...
def evaluate_fold(model_path, test_loader, device, fold, lambda_c=1.0, lambda_p=1e-4):
    """
    Evaluate one trained SCBM model on the test set.
    """

    # Model setup
    backbone = ResNet18Backbone(pretrained=True)
    model = SCBM(backbone=backbone, feature_dim=512, num_concepts=test_loader.dataset.num_concepts,
    ...
    # Criterions
    task_criterion = nn.NLLLoss()

    test_c_loss = 0.0
    test_y_loss = 0.0
    test_total_loss = 0.0
    test_precision_loss = 0.0
    ...
    with torch.no_grad():
        ...
        for batch in test_bar:
            ...
            concept_probs_samples = outputs["concept_probs_samples"]
            class_logits = outputs["class_logits"]
            Sigma = outputs["Sigma"]

            precision = torch.linalg.inv(Sigma)
            off_diag = precision.clone()

            idx = torch.arange(model.num_concepts, device=device)
            off_diag[:, idx, idx] = 0

            precision_loss = off_diag.abs().mean()

            # Loss computation
            targets = concepts.float().unsqueeze(-1)
            log_likelihood = (targets * torch.log(concept_probs_samples + 1e-8)
                +
                (1 - targets) * torch.log(1 - concept_probs_samples + 1e-8))

            log_likelihood = log_likelihood.sum(dim=1)

            c_loss = (math.log(model.mc_samples) - torch.logsumexp(log_likelihood, dim=1)).mean()
            y_loss = task_criterion(class_logits, labels)
            
            loss = (y_loss + lambda_c * c_loss + lambda_p * precision_loss)
            
            # Running losses
            test_c_loss += c_loss.item()
            test_y_loss += y_loss.item()
            test_total_loss += loss.item()
            test_precision_loss += precision_loss.item()

            # Predictions
            c_preds = (concept_probs > 0.5)
            y_preds = class_logits.argmax(dim=1)
    ...
    test_precision_loss_epoch = (test_precision_loss / len(test_loader))
    ...
    print("=" * 60)
    print(
        f"Loss: {test_total_loss_epoch:.4f} "
        f"(C: {test_c_loss_epoch:.4f}, "
        f"Y: {test_y_loss_epoch:.4f}, "
        f"P: {test_precision_loss_epoch:.4f})"
    )
    ...
    return {
            ...
            "test_precision_loss": test_precision_loss_epoch,
...
def main(batch_size=32, SEED=42, n_folds=5, dataset_name="cub"):
    """
    Evaluate all SCBM cross-validation folds.
    """
    ...
    RESULTS_DIR = f"results/{dataset_name}/scbm_cv/seed{SEED}"
    SUMMARY_DIR = f"results/{dataset_name}/scbm_cv"
    ...
    print(f"{dataset_name.upper()} TESTING SCBM_CV MODEL")
    ...
    results_df.to_csv(os.path.join(SUMMARY_DIR, f"test_results_scbm_cv_seed{SEED}.csv"), index=False)
    pd.DataFrame(summary_rows).to_csv(os.path.join(SUMMARY_DIR, f"test_summary_scbm_cv_seed{SEED}.csv"), index=False)

    summary_txt = os.path.join(SUMMARY_DIR, f"test_summary_scbm_cv_seed{SEED}.txt")
