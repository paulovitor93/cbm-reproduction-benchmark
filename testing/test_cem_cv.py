...
def evaluate_fold(model_path, test_loader, device, fold, lambda_c=1.0):
    """
    Evaluate one trained CEM model on the test set.
    """

    # Model setup
    backbone = ResNet18Backbone(pretrained=True)
    model = CEM(backbone=backbone, num_concepts=test_loader.dataset.num_concepts, 
...
def main(batch_size=32, SEED=42, n_folds=5, dataset_name="cub"):
    """
    Evaluate all CEM cross-validation folds and summarize the results.
    """
    ...
    SUMMARY_DIR = f"results/{dataset_name}/cem_cv"
    ...
    print(f"{dataset_name.upper()} TESTING CEM_CV MODEL")
    ...
    results_df.to_csv(os.path.join(SUMMARY_DIR, f"test_results_cem_cv_seed{SEED}.csv"), index=False)
    pd.DataFrame(summary_rows).to_csv(os.path.join(SUMMARY_DIR, f"test_summary_cem_cv_seed{SEED}.csv"), index=False)

    summary_txt = os.path.join(SUMMARY_DIR, f"test_summary_cem_cv_seed{SEED}.txt")
