...
def evaluate_fold(model_path, test_loader, device, fold, lambda_c=1.0):
    """
    Evaluate one trained Sequential CBM model on the test set.
    """

    # Model setup
    backbone = ResNet18Backbone(pretrained=True)
    model = CBM(backbone=backbone, num_concepts=test_loader.dataset.num_concepts,
                num_classes=test_loader.dataset.num_classes,).to(device)
...
def main(batch_size=32, SEED=42, n_folds=5, dataset_name="cub"):
    """
    Evaluate all Sequential CBM cross-validation folds and summarize the results.
    """
    ...
    SUMMARY_DIR = f"results/{dataset_name}/sequential_cv"
    ...
    print(f"{dataset_name.upper()} TESTING SEQUENTIAL_CV MODEL")
    ...
    for fold in range(1, n_folds + 1):
        ...
        model_path = os.path.join(RESULTS_DIR, f"sequential_cbm_fold{fold}_seed{SEED}.pth")
    ...
    results_df.to_csv(os.path.join(SUMMARY_DIR, f"test_results_sequential_cv_seed{SEED}.csv"), index=False)
    pd.DataFrame(summary_rows).to_csv(os.path.join(SUMMARY_DIR, f"test_summary_sequential_cv_seed{SEED}.csv"), index=False)

    summary_txt = os.path.join(SUMMARY_DIR, f"test_summary_sequential_cv_seed{SEED}.txt")
    with open(summary_txt, "w") as f:
        f.write("PER-FOLD RESULTS\n\n")

        for _, row in results_df.iterrows():

           f.write(f"Fold {int(row['fold'])}:\n")
           f.write(f"Y Acc={100*row['test_y_acc']:.2f}%\n")
           f.write(f"Y Precision={100*row['test_y_precision']:.2f}%\n")
           f.write(f"Y Recall={100*row['test_y_recall']:.2f}%\n")
           f.write(f"Y F1={100*row['test_y_f1']:.2f}%\n")

           f.write(f"C Acc={100*row['test_c_acc']:.2f}%\n")
           f.write(f"C Precision={100*row['test_c_precision']:.2f}%\n")
           f.write(f"C Recall={100*row['test_c_recall']:.2f}%\n")
           f.write(f"C F1={100*row['test_c_f1']:.2f}%\n")
