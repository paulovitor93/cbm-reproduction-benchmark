...
def evaluate_fold(model_path, test_loader, device, fold):
    """
    Evaluate one trained ProbCBM classifier on the test set.
    """
    ...
    with torch.no_grad():
        ...
        for batch in test_bar:
            ...
            # Progress bar update
            test_bar.set_postfix(loss=f"{y_loss.item():.4f}", y_acc=f"{100*test_y_correct/max(test_y_total, 1):.2f}%",)
    ...
    print("=" * 60)
    print(f"Y Loss: {test_y_loss_epoch:.4f}")
    print(f"Y Accuracy : {100*test_y_acc:.2f}%")
    print(f"Y Precision: {100*test_y_precision:.2f}%")
    print(f"Y Recall   : {100*test_y_recall:.2f}%")
    print(f"Y F1 Score : {100*test_y_f1:.2f}%")
    print("=" * 60)
...
def main(batch_size=32, SEED=42, n_folds=5, dataset_name="cub"):
    """
    Evaluate all ProbCBM classifier cross-validation folds.
    """
    ...
    SUMMARY_DIR = f"results/{dataset_name}/prob_classifier_cv"
    ...
    print(f"{dataset_name.upper()} TESTING PROB_CBM_CV MODEL")
    ...
    for fold in range(1, n_folds + 1):
        ...
        model_path = os.path.join(RESULTS_DIR, f"prob_classifier_predictor_fold{fold}_seed{SEED}.pth")
    ...
    results_df.to_csv(os.path.join(SUMMARY_DIR, f"test_results_prob_cbm_cv_seed{SEED}.csv"), index=False)
    pd.DataFrame(summary_rows).to_csv(os.path.join(SUMMARY_DIR, f"test_summary_prob_cbm_cv_seed{SEED}.csv"), index=False)
    
    uncertainty_df = pd.DataFrame(uncertainty_info)
    uncertainty_df.to_csv(os.path.join(SUMMARY_DIR, f"uncertainty_prob_cbm_cv_seed{SEED}.csv",), index=False,)

    summary_txt = os.path.join(SUMMARY_DIR, f"test_results_prob_cbm_cv_seed{SEED}.txt")
