...
def evaluate_fold(model_path, test_loader, device, fold):
    """
    Evaluate one concept predictor on the test set.
    """
    ...
    print("=" * 60)
    print(f"Concept Loss: {test_c_loss_epoch:.4f}")
...
def main(batch_size=32, SEED=42, n_folds=5, dataset_name="cub"):
    """
    Evaluate all concept predictors trained by cross-validation.
    """
    random.seed(SEED)
    np.random.seed(SEED)
    ...
    SUMMARY_DIR = f"results/{dataset_name}/concepts_cv"
    ...
    print(f"{dataset_name.upper()} TESTING CONCEPT_CV PREDICTOR")
    ...
    for fold in range(1, n_folds + 1):
        ...
        model_path = os.path.join(RESULTS_DIR, f"concept_predictor_fold{fold}_seed{SEED}.pth")
    ...
    print("\n" + "=" * 80)
    print("CONCEPTS CV TEST RESULTS")
    print("=" * 80)
    ...
    results_df.to_csv(os.path.join(SUMMARY_DIR, f"test_results_concepts_cv_seed{SEED}.csv"), index=False)
    pd.DataFrame(summary_rows).to_csv(os.path.join(SUMMARY_DIR, f"test_summary_concepts_cv_seed{SEED}.csv"), index=False)

    summary_txt = os.path.join(SUMMARY_DIR, f"test_summary_concepts_cv_seed{SEED}.txt")

    with open(summary_txt, "w") as f:
        f.write("CONCEPTS CV TEST RESULTS\n\n")
    ...
    print(os.path.join(RESULTS_DIR, f"test_results_concepts_cv_seed{SEED}.csv"))
